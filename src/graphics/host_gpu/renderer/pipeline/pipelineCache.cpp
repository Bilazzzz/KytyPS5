		...
		ShaderProgram PipelineCache::GetComputeProgram(const HW::ComputeShaderInfo& regs,
			...
			return m_program_cache->Get(params, input_info, push_data_cursor);
												 ...
												 PipelineCache::Pipeline& PipelineCache::CreateGraphicsPipeline(
													 ...
													 // Step 1: Build the key (no lock needed, purely local work + reads of guest state)
													 ...
													 // Step 2: Fast cache lookup under shared_lock (most common path: cache hit)
													 {
													 ...
													 // Step 3: Cache miss. Compilation MUST happen under exclusive lock because
													 // m_driver_cache (vk::PipelineCache) is NOT thread-safe per Vulkan spec.
													 // Multiple threads using the same VkPipelineCache concurrently causes undefined
													 // behavior and possible driver-level corruption.
													 std::unique_lock write_lock(m_mutex);

													 // Step 3a: Re-check — another thread may have compiled the same pipeline while we waited.
													 if (auto iter = m_graphics_pipelines.find(key); iter != m_graphics_pipelines.end()) {
														 return *iter->second;
													 }
									 ...
									 m_driver_cache);
													 ...
													 PipelineCache::CreateComputePipeline(const ShaderComputeInputInfo& input_info,
														 ...
														 // Step 1: Fast cache lookup under shared_lock
														 {
															 std::shared_lock read_lock(m_mutex);
														 ...
														 // Step 2: Cache miss — compilation requires exclusive lock to protect m_driver_cache
														 // (vk::PipelineCache is NOT thread-safe per Vulkan spec).
														 std::unique_lock write_lock(m_mutex);

														 // Step 2a: Re-check — another thread may have compiled it while we waited.
														 if (auto iter = m_compute_pipelines.find(compute_program.id);
															 iter != m_compute_pipelines.end()) {
															 return *iter->second;
															 }
